"""
downstream_tkpi_natural_nb.py — Downstream evaluation on TKPI (natural
missingness) using Gaussian Naive Bayes.

GNB assumes features are normally distributed within each class — a good
complement to RF (nonparametric) and LR (linear). It explicitly models
per-class covariance, so methods that distort the variance structure should
be penalised more visibly.

Output: downstream_tkpi_natural_nb.csv
        downstream_tkpi_natural_nb_summary.csv
"""
import time
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    csv_path = EXP_TABLES / "downstream_tkpi_natural_nb.csv"
    if csv_path.exists():
    ...
    print(f"\n--- Summary (TKPI natural, NB classifier, all {X_raw.shape[0]} rows) ---")
    print(f"Oracle (complete-case n={Xcc.shape[0]}): "
          f"acc={oracle_acc:.4f}  macroF1={oracle_f1:.4f}")
    print(g.round(4).to_string())
    g.to_csv(EXP_TABLES / "downstream_tkpi_natural_nb_summary.csv")
