"""
fig_downstream_clf_f1.py — Downstream classification heatmap (MACRO-F1),
Okabe-Ito sequential colourmap (matching the regression figure).

Macro-F1 is the primary classification metric as of the macro-F1
primary-metric switch (class imbalance on both datasets makes accuracy
misleading; see sec:downstream-protocol). This is the figure included in
the main manuscript as fig_downstream_clf.pdf/png. The accuracy companion
is produced separately by fig_downstream_clf_oi.py for the supplement.

Reads: outputs/tables/downstream_{tkpi,usda}_natural_{,lr_,nb_}summary.csv
Writes: outputs/figures/fig_downstream_clf.{pdf,png}
"""
import sys, os
...
# ── load classification data (macro-F1) ─────────────────────────────────────
# TKPI: RF from natural_summary, LR from natural_lr_summary, NB from natural_nb_summary
tkpi_rf = pd.read_csv(TABLES / "downstream_tkpi_natural_summary.csv")[["method", "clf_f1"]]
tkpi_lr = pd.read_csv(TABLES / "downstream_tkpi_natural_lr_summary.csv")[["method", "clf_f1"]]
tkpi_nb = pd.read_csv(TABLES / "downstream_tkpi_natural_nb_summary.csv")[["method", "clf_f1"]]

tkpi = tkpi_rf.rename(columns={"clf_f1": "rf"}).merge(
    tkpi_lr.rename(columns={"clf_f1": "lr"}), on="method").merge(
    tkpi_nb.rename(columns={"clf_f1": "nb"}), on="method")
tkpi["avg"] = tkpi[["rf", "lr", "nb"]].mean(axis=1)

# USDA: separate RF, LR, NB summary files
usda_rf = pd.read_csv(TABLES / "downstream_usda_natural_rf_summary.csv")[["method", "clf_f1"]]
usda_lr = pd.read_csv(TABLES / "downstream_usda_natural_lr_summary.csv")[["method", "clf_f1"]]
usda_nb = pd.read_csv(TABLES / "downstream_usda_natural_nb_summary.csv")[["method", "clf_f1"]]

usda = usda_rf.rename(columns={"clf_f1": "rf"}).merge(
    usda_lr.rename(columns={"clf_f1": "lr"}), on="method").merge(
    usda_nb.rename(columns={"clf_f1": "nb"}), on="method")
...
cbar.set_label("Macro-F1", fontsize=16, rotation=270, labelpad=18)
cbar.ax.tick_params(labelsize=13)

fig.tight_layout(rect=[0, 0, 0.92, 1])
fs.save(fig, "fig_downstream_clf")
print("Done — saved fig_downstream_clf.pdf + .png (macro-F1)")
